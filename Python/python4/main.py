...
func1(117,'know')
func1(118,'understand the function call')# the value inside the parenthesis 
...
def func6(x=1,y='*',z=1):
  ...
  if y == '+': 
    ...
    if z == 0:
      ...
      return 'expressions need to be one of these: +,-,*,/,%'
...
# function calling external function

def func4(a,b): # called by func3()
  return print(a + b) # returning values from func2()

def func2(a,b): # called directly
  return func3(a,b) # returning value from func2()

def func3(a,b): # called by func2()
  return func4(a,b)

func2('137. ','I started from func2, passed func3 reached func4 and returned here')

# function calling internal function

def func10(): # function called directly
  
  def func11(b): # function called by variable a
    return b
  
  a = func11('from') +" "+ func11('many') +" "+ func11('i') +" "+ func11('was') +" "+ func11('formed')
  return print('138. ',a)
...
# lambda function (arrow function of js)

x = lambda a,b : (a*a + 2*a*b + b*b) 

print('140. ',x(2,3))

# Arrays are not built-in python 
# to use Array 
#import NumPy library

arr = ['one','two','three','four']
arrlen = len(arr)
print('141. ',arr,arrlen)# array

# the rest is same as the lists
...
class Xyz:# class is created using the keyword class followed 
# by the name of the class , use pascal case, followed by colon :
  yz = 2 # property of the class xyz 

obj1 = Xyz() # creating an object of the class Xyz
# since the property of the class is that yz has value 2
print('142. ',obj1.yz) # accessing the value of yz we use the object

class Student:
  def __init__(self,na,id,co): # _init_() function gets
    # automatically called every time a class is created
    # we use this function to define properties of the class by 
    # using the para 'self' of the _init_() function
    self.name = na
    self.id = id
    self.course = co

obj2 = Student('hero','0','nero')

print('143. ',obj2)# object without the _str_() function
print('144. ',obj2.name,obj2.id,obj2.course)# array

class Student1:
  def __init__(self1,na,id,co): 
    # the para 'self' of the _init_() function
    # it does not have to be named 'self' it can be anything, it just 
    # has to be the first para
    self1.name = na
    self1.id = id
    self1.course = co

  def __str__(self):# _str_() function acts as a default value
    # for any object of the class itself belongs to
    # without _str_() the object will give the
    #  o/p <__main__.{class-name} object at {HEX value address}>
    return f"name: {self.name},id: {self.id},course:"
  
  def info(self): # object methods are defined inside the class body
    # similar to any function only with one parameter, best to name it 'self'
    # there can be as many as needed methods defined inside 
    # the arguments are passed from the _init_() function   
    print('145. ',"The Student with ID Number",self.id,"whose name is",self.name,"took the ",self.course," course") 


obj2 = Student1('hero','0','nero')

print('146. ',obj2)# object without the _str_() function

obj2.info() # calling the object method

obj2.name = 'Tosero' # modifying the object properties, now the name has changed
# del obj2.course # course has been deleted

print('147. ',obj2)# object
 
class Empty():
  pass # placeholder for the empty class

# inheritance

class Academics(Student1): # class Academics inherits the class Student1
  # all the properties of the class Student1 can be accessed by the class academics 
  def __init__(self1, na, id, co, excels):# __init__() can be used to define more properties
    # but __init__() function overrides the predefined properties of the inherited class
    # to preserve the inheritance give the parent class a call
    Student1.__init__(self1, na, id, co)  # calling the init of the parent class  
    self1.excels = excels

  def info(self):
    print('148. ',self.name," with id ",self.id," is an excellent",self.excels,"student" )# object

class Sports(Student1):
  def __init__(self1, na, id, co, achi):
    super().__init__(na, id, co) # super function does not need the name of parent
    # it automatically inherits the properties
    self1.achievement = achi
  
  def info(self):
    print('149. ',self.name," with id ",self.id," has an achievement in ",self.achievement )# object

obj3 = Academics('green',9,'science','Essays')
obj4 = Sports('blue',12,'history','marathon')

obj3.info() # calling the academics class object method
obj4.info() # calling the sports class object method

# iterator __iter__() and the __next__() function
...
obj_multiple = Multiplier()
num = iter(obj_multiple)     
...
# module
# save any function you may want to use in many places in a file with .py extension
# lets say module1.py
# to call this module use the keyword import followed by the module name i.e. module1
# to use the function lets say func13() ,we call it as module1.func13()
# modules can also contain all sorts of variables from lists to dict 
# to access the variable inside the module use the same method module1.variable_name
# you can give the module you imported an alias in case module name is large eg., 
# module dictionary_that_we_use_daily.py can be called as
# import dictionary_that_we_use_daily.py as daily_dict
# python has many built in module that can be imported similarly
# dir() function can be used to list all the function inside the module that you imported
# if you want to import a specific function from many functions use 
# from module_name import function_name , here if from is used no need to call the function
#  with the help of the module_name like module1.func13() just call func13
